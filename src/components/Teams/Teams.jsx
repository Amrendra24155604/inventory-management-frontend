  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 px-4 py-8 sm:px-6 lg:px-8">
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="pointer-events-none absolute -bottom-20 -right-24 h-44 w-44 rounded-full bg-indigo-300/30 blur-3xl dark:bg-indigo-600/30" />
        ...
        {/* domain chips */}
        <div className="relative flex flex-wrap justify-center gap-2 sm:gap-3 mb-8 sm:mb-10">
        ...
        {selectedTeam && (
          <section className="relative bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm px-4 py-6 sm:px-6 sm:py-8">
            <h3 className="text-xl sm:text-2xl font-semibold text-center text-slate-900 dark:text-slate-50 mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              {(teams[selectedTeam] || []).map((member) => (
                <Link
                  ...
                  className="group bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4 sm:p-5 rounded-2xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition text-center"
                >
                  <div className="relative w-20 h-20 sm:w-22 sm:h-22 mx-auto mb-3">
                    ...
                    {member.role === "admin" && (
                      <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 text-[10px] sm:text-xs bg-purple-600 text-white rounded-full shadow-sm">
                        Admin
                      </div>
                    )}
                  </div>
                  ...
                  <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                    Roll no: <span className="font-medium">{member.rollNumber}</span>
                  </p>
                </Link>
              ))}
            </div>
