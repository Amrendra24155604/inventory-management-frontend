...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // Auto-detect colors if not specified - made borders crisper and more visible
      const currentBorderColor = borderColor || (isDark ? 'rgba(255, 255, 255, 0.09)' : 'rgba(15, 23, 42, 0.08)');
      const currentHoverFill = hoverFillColor || (isDark ? 'rgba(56, 189, 248, 0.18)' : 'rgba(2, 132, 199, 0.12)');
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ...
              ctx.fillStyle = currentHoverFill;
              ctx.fill();
              ctx.globalAlpha = 1;
            }
      ...
      // Dynamic vignette gradient depending on dark mode - made semi-transparent so background details are visible
      ...
      if (isDark) {
        ...
        gradient.addColorStop(1, 'rgba(3, 7, 18, 0.5)');
      } else {
    ...
    const handleMouseMove = (event) => {
      ...
      // Filter out mouse events when mouse is outside the canvas bounds
    ...
    // Listen globally on window for mouse move to support pointer-events: none layout overlay
